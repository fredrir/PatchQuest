...
hash = ph.hash(password)`,
      ...
      fix(
        ...
        "SHA-256 is fast — exactly what you don't want for passwords. A salt is necessary but not sufficient.",
        { tempting: true },
      ),
      fix(
        "double-md5",
...
if (!await verify(user.hash, password)) {
  return res.status(401).json({ error: "invalid credentials" });
    ...
    explanation:
      "Distinct errors let an attacker enumerate registered emails — useful for credential stuffing, password-reset abuse, and phishing. Always return one generic error and keep timing constant by performing a dummy hash when the email is unknown. The same applies to password-reset and signup endpoints.",
