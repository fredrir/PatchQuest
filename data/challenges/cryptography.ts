...
c2 = encrypt(message2)`,
  ...
  return Math.random().toString(36).slice(2, 10);
    ...
    correctFixId: "csprng",
    explanation:
      "`Math.random()` is a Mersenne-Twister-style PRNG with predictable internal state — observing a few outputs lets an attacker reconstruct the rest. Cryptographic tokens (sessions, password resets, MFA codes) must come from `crypto.randomBytes` or the equivalent CSPRNG.",
